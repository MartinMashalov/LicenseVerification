...
async def get_api_key_by_email(email: str):
    ...
    try:
        ...
        if api_key:
            ...
            raise HTTPException(status_code=404, detail="API key not found for this email")
    except Exception as e:
...
async def get_api_key_by_license(license_key: str):
    ...
    try:
        ...
        if api_key:
            ...
            raise HTTPException(status_code=404, detail="API key not found for this license key")
    except Exception as e:
        logger.error(f"Error retrieving API key for license {license_key}: {e}")
        raise HTTPException(status_code=500, detail="Error retrieving API key")
...
async def create_new_account(
    ...
    try:
        ...
        if user_id:
            ...
            license_key = license_repo.create_and_set_license_key_by_user_id(user_id)
            ...
            if license_key:
                ...
                logger.error(f"License key generation failed for user {user_id}")
                return {
...
async def send_license_email_endpoint(email: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Error sending license email")
...
async def create_checkout_session(
    ...
    try:
        ...
        if user_id:
            # Automatically create license key for new user
            license_key = license_repo.create_and_set_license_key_by_user_id(user_id)
            if license_key:
                logger.info(f"License key {license_key} created for new user {user_email}")
            else:
                logger.error(f"Failed to create license key for new user {user_email}")
        else:
            # User might already exist, update their info instead
            license_repo.update_user_info(
                email=user_email,
                first_name=first_name,
                last_name=last_name,
                company_name=company_name,
        ...
        # Create checkout session with trial period (30 days)
...
async def stripe_webhook(request: Request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        if license_key:
            ...
            email_sender = LicenseEmailSender()
            success, sent_license_key = email_sender.send_license_email(user_email, company_name)
